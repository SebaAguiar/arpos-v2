...
export function CreditNoteDialog({ open, invoice, onClose, onCreated }: CreditNoteDialogProps) {
  ...
  const createCreditNote = useArcaStore((s) => s.createCreditNote);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const amount = amountCents ? Math.round(parseFloat(amountCents) * 100) : undefined;
      if (amountCents && (isNaN(amount!) || amount! <= 0)) {
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "16px" }}>
          <Text size="3" weight="bold">Nota de Crédito</Text>
          <button
            onClick={handleClose}
        ...
        <div style={{ marginBottom: "16px" }}>
          ...
          <Text size="1" color="gray" style={{ display: "block", marginTop: "4px" }}>
            Dejar vacío para monto total ({formatCents(invoice.total_cents)}). El monto no puede superar el total de la factura.
          </Text>
        </div>
        ...
        <FormActions>
          ...
          <Button size="2" color="green" onClick={handleSubmit} disabled={isSubmitting || !reason.trim()}>
            {isSubmitting ? (
