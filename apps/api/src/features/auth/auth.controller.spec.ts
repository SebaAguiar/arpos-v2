...
describe('AuthController', () => {
  ...
  describe('login', () => {
    it('should return token when credentials are valid', async () => {
      const user = { id: 'u1', email: 'admin@arpos.com', name: 'Admin', role: 'admin', companyId: 'c1' };
      mockService.validateUser.mockResolvedValue(user);
      mockService.login.mockResolvedValue({ access_token: 'token-123', user });

      const result = await controller.login({ email: 'admin@arpos.com', password: 'pass' });
    ...
    it('should return error message when credentials are invalid', async () => {
      ...
      const result = await controller.login({ email: 'wrong@arpos.com', password: 'wrong' });
  ...
  describe('getProfile', () => {
    it('should return user profile', async () => {
      const profile = { id: 'u1', email: 'admin@arpos.com', name: 'Admin', role: 'admin' };
