# Arcom Tauri — Estrategia Completa de Actualizaciones
...
```typescript
// apps/arcom-launcher/src/types/updates.ts
...
```json
{
  "updater": {
    ...
    "endpoints": [
      "https://releases.arcom.app/update/{{target}}/{{current_version}}"
    ]
  }
}
```
...
- `windows`: Instala app en C:\Program Files\Arcom (en lugar de roaming)
...
```typescript
// arcom-launcher/src/services/updater.service.ts
...
export class UpdatesController {
  ...
  async checkForUpdates(
    ...
    return {
      needsUpdate: true,
      version: update.version,
      url: `https://releases.arcom.app/download/${update.id}`,
    ...
    checkForUpdates().then((update) => {
      ...
      if (update.critical) {
        ...
        toast.info({
          ...
          description: `Arcom ${update.version} está listo`,
          action: 'Actualizar',
          autoClose: 10000,
        });
      }
    ...
    steps:
      ...
      - name: Upload to S3 (Release Registry)
        run: |
          aws s3 cp \
            src-tauri/target/release/bundle/ \
            s3://arcom-releases/${{ github.ref_name }}/ \
            --recursive
        env:
      ...
      - name: Register Release in Backend
        run: |
          curl -X POST https://api.arcom.app/admin/updates/register \
...
- **`pnpm --filter arcom-launcher tauri build`** compila bundles de todos los targets del
...
```bash
# 1. Ver logs locales
tail -f ~/.arcom/logs/tauri.log
tail -f ~/.arcom/logs/app.log

# 2. Verificar DB
sqlite3 ~/.arcom/data/app.db "SELECT * FROM update_report ORDER BY timestamp DESC LIMIT 5;"

# 3. Forzar check manual
# En settings: "Check for updates"

# 4. Ver última versión registrada
curl https://releases.arcom.app/latest
```
