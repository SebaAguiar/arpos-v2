# Decisiones Técnicas — Arcom Tauri v2

Este documento registra las decisiones arquitectónicas y técnicas tomadas durante la migración de Arcom, incluyendo alternativas consideradas y trade-offs.
...
- **Justificación:** El usuario debe poder usar Arcom 100% offline sin costo de infraestructura. La sincronización es un add-on para quienes necesitan multi-dispositivo.
- **Alternativas consideradas:**
...
- **Alternativas consideradas:**
  ...
  - *Electron Updater:* Rechazado porque Arcom migra a Tauri (decisión #5).
  - *Manual updates (sin auto-updater):* Rechazado porque usuarios no actualizarían, generando soporte técnico innecesario.
  - *Docker-based updates:* Rechazado porque Arcom no usa Docker (decisión #5).
